...
macro "Convert stk to tif" {
	dir=getDir("Choose a Directory containing stk files")
	...
	function processFiles(dir) {
  	...
  	function processFile(path) {
       if (endsWith(path, ".stk")) {
           open(path);
           run("Properties...", "channels=1 slices=1 frames=181 pixel_width=0.1600000 pixel_height=0.16 voxel_depth=0.3 frame=[1 sec]");
           saveAs("tiff",replace(path, ".stk", ".tif"));
           close();
      }
  }
  processFiles(dir)
